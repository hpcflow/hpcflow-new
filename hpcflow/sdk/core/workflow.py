...
from .object_list import TaskList, TaskTemplateList
...
class WorkflowTemplate(JSONLike):
    ...
    _child_objects = (ChildObjectSpec(name="tasks", class_name="Task", is_multiple=True),)
    app = None
...
class Workflow:
    ...
    def __init__(self, history, shared_data, template, tasks):
        self.history = history
        self.shared_data = self.app.shared_data_from_json_like(shared_data)
        self.template = self.app.WorkflowTemplate.from_json_like(
            template, self.shared_data
        )
        self.tasks = tasks

    @classmethod
    def _make_empty_workflow(cls, template: WorkflowTemplate, path=None):
        ...
        if path.is_dir():
            file_name = f"{template.name}_{timestamp.strftime(TS_NAME_FMT)}"
    ...
    @classmethod
    def from_template(cls, template, path=None):
        obj = cls._make_empty_workflow(template, path)
        for task in template.tasks:
            obj.add_task(task)
        return obj

    @classmethod
    def from_YAML_file(cls, YAML_path, path=None):
        template = cls.app.WorkflowTemplate.from_YAML_file(YAML_path)
        return cls.from_template(template, path)

    @classmethod
    def load(cls, path):
        root = zarr.open(path, mode="r")
        obj = cls(
            history=root.attrs["history"],
            shared_data=root.attrs["shared_data"],
            template=root.attrs["template"],
            tasks=root.attrs["tasks"],
        )
        return obj

    def get_possible_input_sources(
        self, schema_input: SchemaInput, new_task: Task, new_index: int
    ):
        """Enumerate the possible sources for an input of a new task, given a proposed
        placement of that task."""
    ...
    def ensure_input_sources(self, new_task: Task, new_index: int):
        """Check valid input sources are specified for a new task to be added to the
        workflow in a given position. If none are specified, set them according to the
        default behaviour."""
    ...
    def add_task(self, task: Task):
        print(f"add_task: {task.name}")
        self.ensure_input_sources(task, len(self.tasks))
